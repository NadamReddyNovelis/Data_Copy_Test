...
4. Provide the prompted inputs as required (_Refer the section below more details on this_).
...
- **`BRANCH`** - **`OPERATION TYPE`** dependancies on **SOURCE ENV** & **TARGET ENV** are as follows:
  - `CLONE`:
    - **FROM_ENV | SOURCE ENV** - Should match the Workspace ENV it needs to be executed in.
  - `CREATE`:
    - **TO_ENV | TARGET ENV** - Should match the Workspace ENV it needs to be executed in.
  - [`PROD`] IN `**PROD**` | [`QA`, `PREPROD`] IN `**QA**` | [`DEV`, `SIT`] IN `**DEV**` 

#### Input Dependencies

- **`TO_ENV`** - **TARGET ENV** dropdown values depend on the branch:
  - **PROD CANNOT BE TARGET ENVIRONMENT**
  - `TO_ENV` can be [**QA**, **PREPROD**, **SIT**, **DEV**]
  - `TO_ENV` can be [**QA**, **PREPROD**, **SIT**, **DEV**]
  - `TO_ENV` can be [**SIT**, **DEV**]
  - **SOURCE ENV SHOULD ALWAYS BE OF A HIGHER ENV THAN TARGET**
    
- **REGION**, **SYSTEM**, and **PLANT** inputs are only applicable if `INGESTION_LAYER` is **SS** (_Source Staging Layer_).
